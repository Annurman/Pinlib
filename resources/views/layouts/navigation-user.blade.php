    ...
    <div class="flex items-center px-4">
        <img src="{{ asset('storage/profile_pictures/logo-pinlib.png') }}" alt="Pinlib Logo" style="width: 60px; height: 60px;" class="ml-4">
...
<div class="relative flex items-center bg-white rounded-lg overflow-hidden shadow-md w-48 md:absolute md:right-4">
                    <a href="{{ route('admin.login') }}" class="flex-1 text-center py-3 transition-all duration-300 bg-gray-700 text-white hover:bg-gray-300 hover:text-black">Admin</a>
                    <a href="{{ route('login') }}" class="flex-1 text-center py-3 transition-all duration-300 bg-green-500 text-white hover:bg-green-300 hover:text-black">User</a>
                </div>
@endif
    ...
    $profileImage = $user->profile && $user->profile->profile_image
        ...
        : $profileImage;
            ...
            <img src="{{ $profileImage }}" alt="Profile Image" class="profile-img w-24 h-24 rounded-full mx-auto border-4 border-white shadow-lg hover:shadow-xl transition duration-300"> <h5>
                @if ($user)
                    {{ $user->name }}
                @elseif ($admin)
                    {{ $admin->name }}
    ...
    <li>
        <a href="{{ route('users.dashboard') }}" class="flex items-center gap-3 px-6 py-3 bg-purple-600 hover:bg-purple-700 rounded-lg shadow-md transition-transform transform hover:scale-105" data-aos="fade-up" data-aos-delay="200">
            <i data-feather="home"></i> Dashboard
        </a>
    </li>
    <li>
        <a href="{{ url('users/profile') }}" class="flex items-center gap-3 px-6 py-3 bg-yellow-600 hover:bg-yellow-700 rounded-lg shadow-md transition-transform transform hover:scale-105" data-aos="fade-up">
            <i data-feather="user"></i> Profile
        </a>
    </li>
    <li>
        <a href="{{ route('member.index') }}" class="flex items-center gap-3 px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg shadow-md transition-transform transform hover:scale-105" data-aos="fade-up">
            <i data-feather="users"></i> Anggota
        </a>
    </li>
    <li>
        <a href="{{ route('users.borrowings.index') }}" class="flex items-center gap-3 px-6 py-3 bg-green-600 hover:bg-green-700 rounded-lg shadow-md transition-transform transform hover:scale-105" data-aos="fade-up" data-aos-delay="100">
